...
async function loadAdmin() {
  ...
  if (pubBtn) {
    pubBtn.textContent = pubEnabled ? 'כבה תשלום ופרסום' : 'הפעל תשלום ופרסום';
    pubBtn.style.background = pubEnabled ? '#c0392b' : 'var(--green)';
    pubBtn.style.color = '#fff';
  }
  if (pubTxt) pubTxt.textContent = pubEnabled ? '✅ תשלום ופרסום פעיל' : '⚠️ תשלום ופרסום מושבת';
...
async function adminTogglePublish() {
  ...
  if (error) { toast('שגיאה: ' + error.message); return; }
  toast(current ? '⚠️ תשלום ופרסום הושבת' : '✅ תשלום ופרסום הופעל');
  await loadAdmin();
}
